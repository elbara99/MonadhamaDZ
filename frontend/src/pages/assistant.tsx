...
const mockConversationData: Omit<Conversation, 'titleKey'>[] = [
  ...
  {
    id: 'conv-2',
    title: 'مقارنة اتجاهات التعليم 2024 مقابل 2025',
    date: '2026-06-27',
    count: 5,
    messages: [],
  },
  {
    id: 'conv-3',
    title: 'توصيات الاستثمار في البنية التحتية',
    date: '2026-06-26',
    count: 12,
    messages: [],
  },
...
function TypingIndicator() {
  return (
    ...
    >
      <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary-100 text-primary-600 dark:bg-primary-900/30 dark:text-primary-400">
        <Bot className="h-4 w-4" />
      </div>
      <div className="flex items-center gap-1.5 rounded-2xl bg-surface-100 px-4 py-3 dark:bg-surface-800">
...
function renderMarkdown(text: string): React.ReactNode {
  ...
  return lines.map((line, i) => {
    if (line.startsWith('**') && line.endsWith('**')) {
      return (
        <p key={i} className="mb-2 text-sm font-semibold text-surface-900 dark:text-white">
          {line.slice(2, -2)}
        </p>
      )
    }
    ...
    if (line.startsWith('*') && line.endsWith('*') && !line.startsWith('**')) {
      return (
        <p key={i} className="mt-3 text-xs italic text-surface-400 dark:text-surface-500">
          {line.slice(1, -1)}
        </p>
      )
    }
    ...
    return (
      <p key={i} className="mb-1 text-sm leading-relaxed text-surface-700 dark:text-surface-300">
        {line}
      </p>
    )
  })
...
export default function AssistantPage() {
  ...
  const [conversationsList, setConversationsList] = useState<Conversation[]>(
    mockConversationData.map((c) => ({
      ...c,
      titleKey: `assistant.${c.id.replace('conv-', 'convTitle')}`,
    })),
  )
    ...
    setTimeout(() => {
      const aiMsg: Message = {
        ...
        dataCards: [
          ...
          { label: 'معدل الإلمام', value: '85.4%', color: 'text-primary-500' },
        ],
      }
      ...
      const finalConv = {
        ...
        count: updatedConv.count + 1,
      }

      setConversationsList((prev) =>
        prev.map((c) => (c.id === selectedConv.id ? finalConv : c)),
      )
      setIsAiTyping(false)
    }, 2000)
  }
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] -mx-4 -mt-6 lg:-mx-8 lg:-mt-8">
      {/* Left Panel — Conversation History */}
      <div className="hidden w-[30%] min-w-[260px] max-w-[340px] flex-col border-r border-surface-200 bg-white dark:border-surface-800 dark:bg-surface-900/80 lg:flex">
        <div className="border-b border-surface-100 p-4 dark:border-surface-800">
          <button
            type="button"
            onClick={handleNewChat}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary-500 px-4 py-2.5 text-sm font-medium text-white shadow-sm transition-all hover:bg-primary-600 hover:shadow-glow"
          >
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedConvId === conv.id
                    ? 'bg-primary-50 shadow-sm dark:bg-primary-950/30'
      ...
      <div className="flex flex-1 flex-col bg-surface-50/50 dark:bg-surface-950/50">
        {selectedConv ? (
          <>
            {/* Messages */}
                ...
                <AnimatePresence mode="popLayout">
                  {selectedConv.messages.map((msg) => (
                    ...
                    >
                      ...
                      >
                        {msg.role === 'assistant' ? (
                          <div className="group overflow-hidden rounded-2xl border border-surface-200/70 bg-white/80 shadow-sm backdrop-blur-xl dark:border-surface-800/50 dark:bg-surface-900/60">
                            ...
                            <div className="px-5 py-4">
                              ...
                              {msg.dataCards && msg.dataCards.length > 0 && (
                                <div className="mt-4 grid grid-cols-3 gap-3">
                                  {msg.dataCards.map((card, i) => (
                                    <div
                                      key={i}
                                      className="rounded-xl border border-surface-100 bg-surface-50/60 px-3 py-3 dark:border-surface-700/50 dark:bg-surface-800/30"
                                    >
                              ...
                              {msg.sources && msg.sources.length > 0 && (
                                <div className="mt-4 flex flex-wrap gap-1.5 border-t border-surface-100 pt-3 dark:border-surface-800">
                          ...
                          <div className="inline-block rounded-2xl bg-primary-500 px-4 py-3 text-sm text-white shadow-sm">
                            {msg.content}
                          </div>
                        )}
                      </div>
            ...
            {/* Input Area */}
            <div className="border-t border-surface-200 bg-white/80 backdrop-blur-xl dark:border-surface-800 dark:bg-surface-900/80">
              <div className="mx-auto max-w-[720px] px-4 py-4 lg:px-8">
                <div className="flex items-end gap-2 rounded-2xl border border-surface-200/70 bg-surface-50 px-4 py-2 shadow-sm transition-all focus-within:border-primary-400 focus-within:shadow-glow dark:border-surface-800/50 dark:bg-surface-900/60 dark:focus-within:border-primary-500/50">
                  <textarea
                    ref={textareaRef}
                    value={inputValue}
                    onChange={(e) => {
                    ...
                    rows={1}
                    className="max-h-[108px] flex-1 resize-none bg-transparent py-2 text-sm text-surface-900 placeholder-surface-400 outline-none dark:text-surface-100 dark:placeholder-surface-500"
                  />
                  <button
                    type="button"
                    onClick={handleSend}
                    disabled={!inputValue.trim()}
                    className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary-500 text-white transition-all hover:bg-primary-600 hover:shadow-glow disabled:opacity-40 disabled:hover:shadow-none"
                  >
                    <Send className="h-4 w-4" />
                  </button>
                </div>
                <p className="mt-2 text-center text-2xs text-surface-400">
                  {t('assistant.aiDisclaimer')}
                </p>
              </div>
            </div>
          </>
          ...
          <div className="flex flex-1 items-center justify-center px-4">
            ...
            >
              <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-primary-100 text-primary-600 dark:bg-primary-900/30 dark:text-primary-400">
                <Bot className="h-8 w-8" />
              </div>
              <h2 className="text-xl font-semibold text-surface-900 dark:text-white">
                {t('assistant.askAboutAlgeria')}
              </h2>
              <p className="mt-2 text-sm text-surface-500 dark:text-surface-400">
                {t('assistant.askDescription')}
              </p>
        ...
        {/* Mobile: show conversation list toggle */}
        <div className="border-t border-surface-200 p-3 lg:hidden dark:border-surface-800">
          <div className="flex gap-2 overflow-x-auto">
            {conversationsList.map((conv) => (
              <button
                ...
                onClick={() => setSelectedConvId(conv.id)}
                className={cn(
                  'shrink-0 rounded-lg px-3 py-2 text-2xs font-medium transition-colors',
                  selectedConvId === conv.id
            ...
            <button
              type="button"
              onClick={handleNewChat}
              className="flex shrink-0 items-center gap-1 rounded-lg bg-primary-500 px-3 py-2 text-2xs font-medium text-white"
            >
